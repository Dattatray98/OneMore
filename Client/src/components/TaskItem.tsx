...
export const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onDelete }) => {
    return (
        <div
            className={`group flex items-center justify-between p-3 md:p-4 mb-2 md:mb-3 rounded-xl backdrop-blur-md border transition-all duration-300 ${task.completed
        ...
        >
            <div className="flex items-center gap-4 flex-1">
                ...
                <span
                    className={`text-base md:text-lg transition-all duration-300 ${task.completed ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-900 dark:text-slate-100'
                        }`}
                >
                    {task.text}
                </span>
